...
public class Cast {
  ...
  public static class CastException extends RuntimeException { public CastException(String n) { super(n); }}
  
  private static Cast singleton = new Cast();
  
  public static <T> T cast(Class<T> t, Object z) { return singleton.doCast(t, z); }
  public static <T> T cast(Type t, Object z) { return singleton.doCast(t,z); }
  public static <T extends Collection<U>,U> T cast(Class<T> t, Class<U> h, Object z) { return singleton.subCast(t, h, z); }
  
  public static Object callStatic(String cl, String mth, Object[] args)
  ...
  public static Object callStatic(Class<?> cl, String mth, Object[] args)
      throws InvocationTargetException, NoSuchMethodException {
  ...
  public static Object callMethod(Object o, String mthnam, Object[] args)
      throws InvocationTargetException, NoSuchMethodException {
    return singleton.callMethod(o.getClass(), o, null, mthnam, args);
  }

  @Retention(RetentionPolicy.RUNTIME)
  @Inherited public static @interface Castable {
  }
  
  @SuppressWarnings("unchecked")
  protected <T> T castInteger(Class<T> t, Integer z) {
    ...
    if (t.isEnum()) {
      ...
      String h = "$"+z;
      for(int i=0;i<ec.length;i++) if (ec[i].toString().equals(h)) return ec[i];
      return ec[z];
    }
  ...
  @SuppressWarnings("unchecked")
  protected <T> T castBoolean(Class<T> t, Boolean z) {
  ...
  @SuppressWarnings("unchecked")
  protected <T> T castDouble(Class<T> t, Double z) {
    if (t == Long.class || t == long.class) return (T)new Long(z.longValue()); 
    if (t == Boolean.class || t == boolean.class) return (T)new Boolean(z == 1);
  ...
  @SuppressWarnings("unchecked")
  protected <T> T castLong(Class<T> t, Long z) {
    if (t == Integer.class || t == int.class) return (T)new Integer(z.intValue()); 
    if (t == Boolean.class || t == boolean.class) return (T)new Boolean(z == 1);
  ...
  @SuppressWarnings("unchecked")
  protected <T> T castString(Class<T> t, String z) {
    ...
    if (t == Boolean.class || t == boolean.class) { if (z == null || z.length() < 1) return (T)new Boolean(false);
      Character k = z.charAt(0);
      return (T)new Boolean(k == '1' ||  k == 't' || k == 'T' || k == 'y' || k == 'Y'); }
    if (t == Integer.class || t == int.class) return z.isEmpty() ? (T)new Integer(Integer.MIN_VALUE) : (T)new Integer(Integer.parseInt(z));
    ...
    if (t.isEnum()) {
      ...
      for(Object nn : t.getEnumConstants() ) if (nn.toString().equalsIgnoreCase(z)) return (T)nn;
      for(int i=0;i<z.length();i++) if (!Character.isDigit(z.charAt(i))) throw new CastException("value "+z+" not member of enum "+t);
  ...
  public <T extends Collection<U>,U> T subCast(Class<T>r, Class<U> h, Object z) {
    T ll = null;
    if (z == null) return null;
    try { ll = r.getConstructor().newInstance(); }
    catch(Exception e) { throw new CastException(" Array => "+r); }

    Type zox; // = z.getClass().getComponentType();
    zox = h;
    if (z instanceof Collection) {
      for( Object o : (Collection)z) ((Collection)ll).add( doCast(zox, o));
  ...
  @SuppressWarnings({ "unchecked", "rawtypes" })
  protected <T> T castArray(Class<T> t, Object z) {
    ...
    if (cc != null) {     
      try { return t.getConstructor(z.getClass()).newInstance(z); }
      catch(Exception ite) { throw new CastException(z.getClass() +" => "+t); }
    }
    
    if (t.isArray()) {
      Class<?>ct = t.getComponentType();
      Object res = Array.newInstance(ct,zl);
      for(int i=0;i<zl; i++) { Array.set(res, i, doCast(ct, Array.get(z,i))); }
      return (T)res;
    }
    
    if (Collection.class.isAssignableFrom(t)) {
      Collection ll = null;
      try { ll = (Collection) t.getConstructor().newInstance(); }
      catch(Exception e) { throw new CastException(" Array => "+t); }

      Type zox = z.getClass().getComponentType();
      Type[] tx = t.getTypeParameters(); //    getActualTypeArguments();
      zox = tx[0];
      for(int i=0;i<zl; i++) ll.add( doCast(zox, Array.get(z,i)));
      return (T)ll;
    }
  ...
  protected Object castToArray(Type ct, Object z) {
    if (z.getClass().isArray()) {
      ...
      int i=0;
       for(Object zz : (Collection)z) { Array.set(res, i++, doCast(ct, zz)); }
       return res;     
    } else throw new CastException(z.getClass()+" => array of "+ct);
  }
  
  @SuppressWarnings("unchecked")
  protected <T> T castNull(Class<T> t) {
  ...
  protected <U,V> Map<U,V> castToMap( Class<? extends Map>r, Class<U>ck, Class<V>cv, Object z ) {
	  ...
	  try { res = (Map<U,V>)r.newInstance(); } catch(IllegalAccessException x) {} 
	  catch(InstantiationException x) { throw new CastException(x.toString()); }
  ...
  @SuppressWarnings("unchecked")
  protected <T> T doCast(Type t, Object z) { 
    if (t instanceof ParameterizedType) {
      Type r = ((ParameterizedType)t).getRawType();
      Type[] h = ((ParameterizedType)t).getActualTypeArguments();
      if ( Map.class.isAssignableFrom( (Class<?>)r )) {
    	  return (T) castToMap((Class<? extends Map>)r, (Class<?>)h[0],(Class<?>)h[1],z);
      }
  ...
  @SuppressWarnings("unchecked")
  protected <T> T doCast(Class<T> t, Object z) {
    ...
    if (cc != null) {     
      try {
          ...
          for(Constructor<?> c : cos) {
            ...
            if (c.getParameterTypes()[0].isAssignableFrom(z.getClass())) {
              return (T) c.newInstance(z);
            }
          }
      ...
      catch(InvocationTargetException e) { throw new CastException( z.getClass()+" => "+t+" ("+e.getCause().getMessage()+")"); }
      catch(InstantiationException e) { throw new CastException( z.getClass()+" => "+t+" ("+e.getMessage()+")"); }
    }
    ...
    if ( Map.class.isAssignableFrom(t)) {
       if (Map.class.isAssignableFrom(z.getClass())) {
        ...
        try { 
            ...
            for(Field f : fs ) {res.put(f.getName(), f.get(z)); }
            return (T)res;
        } catch(IllegalAccessException iae) {} catch(InstantiationException ie) {}
       }
    }
  ...
  protected Object callMethod(Class<?> clazz, Object o, Class<? extends Annotation>annot, String mthnam, Object[] args)
    ...
    for(Method m: mth) {
      ...
      for(i=0;i<pt.length;i++) {
        ...
        if (pt[i].isPrimitive()) {
          Object c = null;
          try { c = methargs[i].getClass().getDeclaredField("TYPE").get(null); }
          catch(NoSuchFieldException nsfe) {}
      ...
      } catch(IllegalAccessException iae) { continue; }
    }
    StringBuffer sb = new StringBuffer();
    for(Object a : args) sb.append(","+a.getClass());

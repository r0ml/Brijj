...
public class Json {
  ...
  private static Map<Character, String> replacements = new HashMap<Character, String>();
  static {
  ...
  @SuppressWarnings("unchecked") public static void writeObject(Object xa, Writer w) throws IOException {
    Object x = xa;
    ...
    if (x instanceof String) {
      w.write("\"" + escapeJavaScript((String) x) + "\"");
      return;
    }
    if (x instanceof Calendar) x = ((Calendar) x).getTime();
    if (x instanceof Date) {
      w.write("new Date(" + ((Date) x).getTime() + ")");
      return;
    }
